...
export default function Notifications({
   ...
   // Redux : Global State
   const globalNotification = useSelector(state => state.modal.notification)
   const dispatch = useDispatch()

   // Local State
   const [show, setShow] = useState(isNotification)
   const [notification, setNotification] = useState({
      isNotification: isNotification || globalNotification.show,
      isSuccess: isSuccess || globalNotification.isSuccess,
      message: message || globalNotification.message,
      description: description || globalNotification.description,
   })
   ...
   useEffect(() => {
      setNotification({
         isNotification: globalNotification.show,
         isSuccess: globalNotification.isSuccess,
         message: globalNotification.message,
         description: globalNotification.description,
      })
      setShow(globalNotification.show)
   }, [globalNotification])
   return (
      <>
         ...
         <div
            aria-live='assertive'
            className='fixed right-0 bottom-0 w-full  sm:w-[30rem] sm:left-160 flex flex-row justify-end items-end px-4 py-6 pointer-events-none sm:p-6 sm:items-start z-30'>
